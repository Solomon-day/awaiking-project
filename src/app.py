...
tab1_gold = [
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            html.P('''График выше отражает соотношение унций золота к одному лоту газа.
...
tab2_bitcoin = [
    dbc.Row([
        dbc.Col([
            ...
            html.P('''Данный график, как и соседний, показывает общие среднегодовые расходы американца, но уже по отношению не к доллару, а к bitcoin (включая траты на ипотеку, здравоохранение, 
    ...
    dbc.Row([
        dbc.Col([
            ...
            html.P('''Данный график показывает стоимость газа (одного биржевого лота фьючерса на газ) в bitcoin. То есть шкала Y показывает колличество необходимых монет bitcoin.''')
        ],
            width={'size': 6}),
        dbc.Col([
            dcc.Graph(figure=fig_gas_usd),
            html.P('''Данный график показывает стоимость газа (одного биржевого лота фьючерса на газ) в долларах США.''')
        ],
            width={'size': 6})
    ]),
    # dbc.Row([
...
tab4_inflation = [
    ...
    dbc.Row([
        ...
        html.P('''На графике отображена динамика стоиомсти золота в долларах США.Примечательно то, что до отмены Бреттон-Вудской системы в ~1976 году цена на золото практически не изменялась.
...
tab3_about = [
    ...
    завершенным и будет дорабатываться.'''),
             ...
             программным одом и не может быть изменена, как я и писал ранее, можно рассчитать затраты и финансовую 
